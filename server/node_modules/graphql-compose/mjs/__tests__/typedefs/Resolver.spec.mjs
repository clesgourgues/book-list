"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});

var Resolver_1 = require("../../Resolver");

var findManyPost = new Resolver_1.Resolver({
  resolve: function (rp) {
    if (rp.source && rp.context) {
      // rp.source.timestamp = 'fails';
      rp.source.timestamp = 4;
      rp.source.author = 'GraphQL superb with Compose';
    }
  }
});
var findOnePostAny = new Resolver_1.Resolver({
  resolve: function (rp) {
    if (rp.source && rp.context) {
      // rp.source.timestamp = 'fails';
      rp.source.timestamp = 'compose';
      rp.source.author = 4;
    }
  }
}); // wrap

var findManyArt = findManyPost.wrap(function (newResolver, prevResolver) {
  newResolver.cloneArg('filter', 'AuthorFilterForUsers');
  newResolver.getArgTC('filter').removeField(['age', 'other_sensetive_filter']);
  return newResolver;
}, {
  resolve: function (_a) {
    var source = _a.source,
        context = _a.context;

    if (source && context) {
      // source.id = 'string' fails
      source.id = 444;
    }
  }
});
findManyArt.wrapResolve(function (next) {
  return function (rp) {
    if (rp.source && rp.context) {
      // rp.source.id = 'string' fails
      rp.source.id = 444;
    }
  };
});
findManyPost.wrapResolve(function (next) {
  return function (rp) {
    if (rp.source && rp.context) {
      // rp.source.title = 555 fails
      rp.source.title = 'A Title';
    }
  };
});
var findManyArt1 = new Resolver_1.Resolver({
  resolve: function (rp) {
    if (rp.args) {
      // rp.args.skip = 'dsd';
      rp.args.skip = 4;
      rp.args.filter.id = 'string';
    }
  }
}); // all any

var findManyPost1 = new Resolver_1.Resolver({
  resolve: function (_a) {
    var source = _a.source,
        context = _a.context,
        args = _a.args;
    source.title = 555; // pass as source is any fails

    source.title = 'A Title';

    if (args) {
      args.skip = 'hello';
      args.skip = 4;
    }
  }
}); // inherits findManyArtArgs, can be overwritten

findManyArt1.wrapResolve(function (next) {
  return function (rp) {
    if (rp.source && rp.context && rp.args) {
      // rp.source.id = 'string' fails
      rp.source.id = 444; // rp.args.skip = 'string';

      rp.args.skip = 4;
    }
  };
});
findManyArt1.wrap(function (newResolver, prevResolver) {
  newResolver.cloneArg('filter', 'AuthorFilterForUsers');
  newResolver.getArgTC('filter').removeField(['age', 'other_sensetive_filter']);
  return newResolver;
}, {
  resolve: function (_a) {
    var source = _a.source,
        context = _a.context,
        args = _a.args;

    if (source && context && args) {
      // source.id = 'string' fails
      source.id = 444; // args.skip = 'fails';

      args.skip = 3;
    }
  }
});